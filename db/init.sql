...
--
-- This file is the complete schema at HEAD. Baked into the impact-score-db
-- image and consumed by MySQL on first container start via /docker-entrypoint-initdb.d/.
--
-- Two-file rule (see README → Contributing): every schema change must touch
-- both this file AND a new file in db/migrations/. The schema_migrations
-- pre-population at the end of this file makes fresh installs skip migrations
-- that are already baked into the init schema.
-- ─────────────────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS scores (
    ...
    q4  TINYINT DEFAULT 0, q5  TINYINT DEFAULT 0, q6  TINYINT DEFAULT 0,
    q7  TINYINT DEFAULT 0, q8  TINYINT DEFAULT 0, q9  TINYINT DEFAULT 0,
    q10 TINYINT DEFAULT 0, q11 TINYINT DEFAULT 0, q12 TINYINT DEFAULT 0,
    q13 TINYINT DEFAULT 0, q14 TINYINT DEFAULT 0, q15 TINYINT DEFAULT 0,
    q16 TINYINT DEFAULT 0, q17 TINYINT DEFAULT 0, q18 TINYINT DEFAULT 0,
    q19 TINYINT DEFAULT 0, q20 TINYINT DEFAULT 0, q21 TINYINT DEFAULT 0,
    q22 TINYINT DEFAULT 0, q23 TINYINT DEFAULT 0, q24 TINYINT DEFAULT 0,
    q25 TINYINT DEFAULT 0, q26 TINYINT DEFAULT 0, q27 TINYINT DEFAULT 0,
    q28 TINYINT DEFAULT 0, q29 TINYINT DEFAULT 0, q30 TINYINT DEFAULT 0,
...
-- Admin accounts
--
-- NOTE: Fresh installs ship with NO admin account. Use `make seed-dev` for
-- a dev-only admin, or wire up the BOOTSTRAP_ADMIN_TOKEN flow for production.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Reporting / programs / calendar / dynamic columns
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Custom Dropdown Fields (introduced by migration 001_add_custom_fields.sql)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Reference seed data (kept in init.sql by design — these are defaults, not
-- test data. Test/sample data lives in db/seed_dev.sql.)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Seed: Age Group custom field (folded in from migration 001). Idempotent.
...
-- Migration tracking
--
-- Pre-populate schema_migrations with every migration whose changes are
-- already folded into the schema above. On fresh installs, the runner sees
-- these entries and skips the corresponding files, so a `make up` after init
-- prints `[migrate] Done. Applied: 0 migration(s).`.
-- ─────────────────────────────────────────────────────────────────────────────
